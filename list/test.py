...
ADMIN_BOARD_1 = 'admin-board #1'
USER_BOARD_1 = 'user-board #1'
...
class WorkingBoardListPage(TestCase):
    ...
    def test_create_board(self):
        ...
        self.client.post(reverse('create_board'), {'title': 'Board create by form', 'color': '#AAABBB'})
...
class WorkTaskListPage(TestCase):
    def setUp(self):
        ...
        admin.is_moderator = True
        admin.save()
        admin_board = admin.board_set.create(title=ADMIN_BOARD_1)
        user = User.objects.get(email=USER_EMAIL)
        user_board = user.board_set.create(title=USER_BOARD_1)
...
class WorkingDetailTaskPage(TestCase):
    ...
    def test_edit_task(self):
        ...
        self.client.post(reverse('edit_task', kwargs={'board_pk': user_board.id,
                         ...
                         {'description': 'new_description',
                          'scheduled_deadline': '2020-12-12', 'task_status': 'completed'})
        user_task = user_board.task_set.all()[0]
        ...
        self.assertURLEqual(response.redirect_chain[0][0], reverse('edit_task', kwargs={'board_pk': user_board.id,
                                                                                        'task_pk': user_task.id}))
        messages = list(response.context['messages'])
        self.assertEqual(str(messages[0]), 'Input deadline !!!')
        ...
        self.assertURLEqual(response.redirect_chain[0][0], reverse('edit_task', kwargs={'board_pk': user_board.id,
                                                                                        'task_pk': user_task.id}))
        messages = list(response.context['messages'])
        self.assertEqual(str(messages[0]), 'Deadline should be today or later')
    ...
    def test_get_replace_copy_task_page(self):
        ...
        response = self.client.get(reverse('replace_task', kwargs={'board_pk': admin_board.id,
